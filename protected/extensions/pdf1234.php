...
class pdf1234{
	...
	protected function text_manyholes($models){
		...
		//$ar['holes'][] = $model->type->name.', '.$model->ADDRESS.($model->COMMENT1 ? ', '.trim($model->COMMENT1) : '').', http://rosdostup.ru'.CController::createUrl("/holes/view",Array('id'=>$model->ID))."";    
		$ar['holes'][] = $model->type->name.', '.$model->ADDRESS.($model->COMMENT1 ? ', '.trim($model->COMMENT1) : '').', http://'.$_SERVER['HTTP_HOST']'.CController::createUrl("/holes/view",Array('id'=>$model->ID))."";    
		}
