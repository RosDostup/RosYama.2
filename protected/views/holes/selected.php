		...
		<div class="info">
			...
			<div class="control">			
				<div class="progress">
							...
							<div class="cc">
							<?php echo CHtml::link('Пометить как отправленное '.Y::declOfNum(count($list->notSentHoles),Array('дефект','дефекта','дефектов')), Array('sentMany','holes'=>implode(',',CHtml::listData($list->notSentHoles,'ID','ID'))), Array('class'=>'')); ?><br/>
							</div>
							<?php endif; ?>
							<?php if ($list->sentedHoles) : ?>
							<div class="rc" style="padding: 24px 15px;">
							<?php echo CHtml::link('Загрузить ответ на '.Y::declOfNum(count($list->sentedHoles),Array('дефект','дефекта','дефектов')), Array('gibddreply','holes'=>implode(',',CHtml::listData($list->sentedHoles,'ID','ID'))), Array('class'=>'')); ?><br/>
							</div>
							<?php endif; ?>
