<?
$this->pageTitle=Yii::app()->name . ' - FAQ';
$this->layout='//layouts/header_default_without_add';
?>
<div class="lCol">
<?php $this->widget('application.widgets.news.newsWidget'); ?>
<?php $this->widget('application.widgets.social.socialWidget'); ?>
</div>
<div class="rCol">
<h1>Вопросы и ответы</h1>
	<table class="faq-item">
		<tr>
			<th>Q:</th>
			<td class="question">Где можно получить больше информации, пообщаться с единомышленниками, и т. д?</td>
		</tr>		
		<tr>
			<th>A:</th>
			<td>
			<p>Почта: <a href="mailto:rossyama.gmail.com">rossyama@gmail.com</a></p>
                        <p>Форум: <a href="http://forum.rosyama.ru">http://forum.rosyama.ru</a></p>
                        <p>Твиттер: <a href="https://twitter.com/#!/fezeev">https://twitter.com/#!/fezeev</a></p>
                        <p>LiveJournal: <a href="http://fezeev.livejournal.com/">http://fezeev.livejournal.com/</a></p>
                        </td>
		</tr>		
	</table>	

	<table class="faq-item">
		<tr>
			<th>Q:</th>
			<td class="question">Где я могу скачать мобильные приложения для проекта «<?=Yii::app()->name;?>» под IPhone или Android?</td>
		</tr>		
		<tr>
			<th>A:</th>
			<td><p>Приложение для IPhone: <a href="http://itunes.apple.com/ru/app/id456229487?mt=8">http://itunes.apple.com/ru/app/id456229487?mt=8</a></p>
			    <p>Приложение для Android от UnrealMojo: <a href="https://market.android.com/details?id=ru.rosyama.android">https://market.android.com/details?id=ru.rosyama.android</a></p>
			    <p>Приложение для Android от RedSolution: <a href="https://market.android.com/details?id=ru.redsolution.rosyama">https://market.android.com/details?id=ru.redsolution.rosyama</a></p>
			    <p>Приложение для Samsung Bada: <a href="http://www.samsungapps.com/topApps/topAppsDetail.as?productId=G00004062550">http://www.samsungapps.com/topApps/topAppsDetail.as?productId=G00004062550</a></p>
			    <p>Приложение для Windows Phone: <a href="http://www.windowsphone.com/ru-RU/apps/783e000f-c9ac-4c11-af17-f5e465cd881b">http://www.windowsphone.com/ru-RU/apps/783e000f-c9ac-4c11-af17-f5e465cd881b</a></p>
			    <p>Ещё одно приложение для Windows Phone: <a href="http://www.windowsphone.com/ru-RU/apps/8994aa3e-3685-4d8e-8489-d1913591b7c3">http://www.windowsphone.com/ru-RU/apps/8994aa3e-3685-4d8e-8489-d1913591b7c3</a></p>
			</td>

		</tr>		
	</table>	

	<table class="faq-item">
		<tr>
			<th>Q:</th>
			<td class="question">Что делать, если на дороге настолько много дефектов, что впору снимать видео, а не фото?</td>
		</tr>		
		<tr>
			<th>A:</th>
			<td>
			<p>Сфотографируйте один дефект, на ваш выбор. Поставьте точку на сайте «<?=Yii::app()->name;?>», получите текст заявления. 
			Полученный текст немного поправьте — напишите не "обнаружен дефект по адресу", а "обнаружены множественные повреждения дорожного покрытия на всём протяжении местности [название местности] от [пункт А] до [пункт Б]".</p>
			<p>А дальше всё то же самое.</p>
			</td>

		</tr>		
	</table>	

	<table class="faq-item">
		<tr>
			<th>Q:</th>
			<td class="question">Почему бы не создать такой же сайт по другой проблемной тематике (жкх, помойки, наркотики, выборы, и т. д.)</td>
		</tr>		
		<tr>
			<th>A:</th>
			<td>Я всячески поддерживаю идею создания аналогичных сайтов по другим проблемам.<br />
В частности, исходный код нашего сайта доступен на условиях лицензии BSD, так что любой желающий может ими воспользоваться для создания чего-то аналогичного или похожего. Подробности можно узнать по ссылке <a class="txttohtmllink" href="http://forum.rosyama.ru/viewtopic.php?f=4&t=81">http://forum.rosyama.ru/viewtopic.php?f=4&t=81</a><br />
<br />
Однако я не могу распылять свои силы одновременно на много проблем. Ибо в таком случае я не смогу сделать вообще ничего. Я хочу сконцентрироваться именно на безопасных дорогах, чтобы добиться результата именно в этой области.<br />
<br />
Найдите единомышленников и вместе с ними решайте те проблемы, которые волнуют именно Вас. Не надо ждать, пока кто-то другой решит их за Вас. Даже один человек может сделать очень многое. <br />
<br />
В последнее время в сети возникает много сайтов, объединяющих людей по волнующим их проблемам. Например <a class="txttohtmllink" href="http://domdvordorogi.ru/">http://domdvordorogi.ru/</a> - по проблемам ЖКХ.</td>
		</tr>		
	</table>	
	<table class="faq-item">
		<tr>
			<th>Q:</th>
			<td class="question">Как лучше отправлять заявления о нескольких дефектах в одно подразделение местных внутренних органов?</td>
		</tr>		
		<tr>
			<th>A:</th>
			<td>В такой ситуации лучше всего объединить несколько дефектов в одно заявление.<br />
Поскольку на каждое заявление органы должны отвечать отдельно, такое объединение нескольких дефектов в одном заявлении поможет им не заниматься бессмысленным бумагомаранием и оставит им больше времени на реальную работу.</td>
		</tr>		
	</table>	
<!--	<table class="faq-item">-->
<!--		<tr>-->
<!--			<th>Q:</th>-->
<!--			<td class="question">Что делать, если из-за ямы я попал в ДТП? Как возместить ущерб?</td>-->
<!--		</tr>		-->
<!--		<tr>-->
<!--			<th>A:</th>-->
<!--			<td>Подробный разбор ДТП из-за ямы есть в ЖЖ Георгия Бадьина (в двух частях). Часть <a href="http://wilych96.livejournal.com/175051.html">первая</a>. Часть <a href="http://wilych96.livejournal.com/175278.html">вторая</a>.<br>-->
<!--			Еще один вариант разбора от Андрея Сычикова: Часть <a href="http://grajdanin25.livejournal.com/1385.html">первая</a>. Часть <a href="http://grajdanin25.livejournal.com/1730.html">вторая</a>.-->
<!--			</td>-->
<!--		</tr>		-->
<!--	</table>	-->
</div>